...
def available_under(menu: list[dict], limit: float) -> list[dict]:
    ...
    # DONE: filter items priced under $10, then sort by price.
    belowTenItems = []
    for i in menu:
        if i["price"] < limit:
            belowTenItems.append(i)

    return sorted(belowTenItems, key=lambda i: i["price"])
...
def main() -> None:
    ...
    for item in available_under(menu, 10.00):
        ...
        # DONE: print name and price using an f-string.
